...
public class ScoreboardController extends Stage {
  ...
  public void startTime() throws GameNotStartedException {
    ...
    TimerTask task = new TimerTask() {
      ...
      @Override
      public void run() {
        if (actualTime.get() == GameSettingsProvider.THIRD_DURATION.get() && timeState == TimeState.IN_HALF) {
          actualTime.set(GameSettingsProvider.PAUSE_DURATION.get());
          timerCountUp = false;
          timeState = TimeState.PAUSE;
          stopTime();
          return;
        } else if (actualTime.get() == 0 && timeState == TimeState.PAUSE) {
        ...
        if (timerCountUp) {
          actualTime.setValue(actualTime.getValue() + 1000);
        } else {
          actualTime.setValue(actualTime.getValue() - 1000);
        }
      }
